...
public static class ConverterLogic
{
    ...
    public static IEnumerable<string> SvgFilesFromFolder(string folder, bool handleSubFolders = false)
    {
        try
        {
            return handleSubFolders ?
                Directory.GetFiles(folder, "*.svg*", SearchOption.AllDirectories)
                : (IEnumerable<string>) Directory.GetFiles(folder, "*.svg*");
        }
        catch
        {
    ...
    private static void AddDrawingImagesToDrawingGroups(XElement rootElement)
    {
        ...
        foreach (XElement node in drawingGroups)
        {
            ...
            XElement drawingImage = new(NsDef + "DrawingImage",
    ...
    public static void SizeGeometry(PathGeometry pg, Size size)
    {
        if (size.Height > 0)
        {
            ...
            PathGeometry newGeo = new(sizeFigures.Concat(pg.Figures), pg.FillRule, null);//pg.Transform do not add transform here, it will recalculate all the Points
            pg.Clear( );
    ...
    internal static DrawingGroup SvgFileToWpfObject(string filepath, WpfDrawingSettings wpfDrawingSettings)
    {
        ...
        FileSvgReader reader = new(wpfDrawingSettings);
    ...
    {
        XmlXamlWriter writer = new(new WpfDrawingSettings { IncludeRuntime = includeRuntime });
